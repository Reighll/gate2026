<div class="modal fade sheet-modal" id="editGuardModal<?= $guard['id'] ?>" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered px-2 px-sm-0">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 24px;">

            <form action="<?= base_url('admin/users/editGuard/' . $guard['id']) ?>" method="post">
                <?= csrf_field() ?>

                <div class="modal-header border-bottom-0 pt-4 px-4 pb-2 d-flex justify-content-between align-items-center">
                    <h5 class="modal-title fw-bolder d-flex align-items-center mb-0 text-truncate pe-2">
                        <div class="rounded-circle d-flex align-items-center justify-content-center me-3 flex-shrink-0" style="width: 40px; height: 40px; background-color: rgba(255, 174, 31, 0.15); color: #ffae1f;">
                            <i class="ti ti-shield-lock fs-5"></i>
                        </div>
                        Edit Guard Details
                    </h5>
                    <div class="d-flex align-items-center gap-2 flex-shrink-0">
                        <button type="button" class="btn-close shadow-none m-0" data-bs-dismiss="modal" aria-label="Close"></button>
                        <button type="button" class="mobile-sheet-close" data-bs-dismiss="modal" aria-label="Close">
                            <i class="ti ti-x"></i>
                        </button>
                    </div>
                </div>

                <div class="modal-body px-4 py-3">
                    <div class="mb-3">
                        <label class="form-label fw-bold text-muted" style="font-size: 0.75rem; letter-spacing: 0.5px; text-transform: uppercase;">Username</label>
                        <input type="text" class="form-control form-control-lg rounded-3 input-adaptive shadow-none" name="username" value="<?= esc($guard['username']) ?>" required>
                    </div>
                    <div class="row">
                        <div class="col-sm-6 mb-3">
                            <label class="form-label fw-bold text-muted" style="font-size: 0.75rem; letter-spacing: 0.5px; text-transform: uppercase;">First Name</label>
                            <input type="text" class="form-control form-control-lg rounded-3 input-adaptive shadow-none" name="first_name" value="<?= esc($guard['first_name']) ?>" required>
                        </div>
                        <div class="col-sm-6 mb-3">
                            <label class="form-label fw-bold text-muted" style="font-size: 0.75rem; letter-spacing: 0.5px; text-transform: uppercase;">Last Name</label>
                            <input type="text" class="form-control form-control-lg rounded-3 input-adaptive shadow-none" name="last_name" value="<?= esc($guard['last_name']) ?>" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-bold text-muted" style="font-size: 0.75rem; letter-spacing: 0.5px; text-transform: uppercase;">Reset Password</label>
                        <div class="position-relative">
                            <input type="password" class="form-control form-control-lg rounded-3 input-adaptive shadow-none pe-5" name="password" placeholder="Leave blank to keep current password">
                            <button type="button" class="btn toggle-password-btn position-absolute top-50 end-0 translate-middle-y me-2 p-0 text-muted" tabindex="-1" style="z-index: 5; background: none; border: none;">
                                <i class="ti ti-eye"></i>
                            </button>
                        </div>
                    </div>
                </div>

                <div class="modal-footer border-top-0 px-4 pb-4 pt-2 d-flex justify-content-end modal-actions-mobile">
                    <button type="submit" class="btn btn-warning text-white fw-bold px-4 py-2 shadow-sm m-0" style="border-radius: 10px; border: none;">Save Changes</button>
                </div>
            </form>
        </div>
    </div>
</div>