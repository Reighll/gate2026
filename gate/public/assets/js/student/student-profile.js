...
if (!window.__profileCropperDelegated) {
    window.__profileCropperDelegated = true;
    ...
    function getCropModal() {
        ...
        return bootstrap.Modal.getInstance(cropModalEl) || new bootstrap.Modal(cropModalEl);
    }

    document.body.addEventListener('change', async function (e) {
        if (!e.target || e.target.id !== 'profile_pic') return;
    ...
    document.body.addEventListener('shown.bs.modal', function (e) {
        if (!e.target || e.target.id !== 'cropModal') return;
        const cropperImage = document.getElementById('cropperImage');
    ...
    document.body.addEventListener('hidden.bs.modal', function (e) {
        ...
        if (previewImage && previewImage.getAttribute('data-updated') !== 'true' && fileInput) {
            fileInput.value = '';
        }
        if (previewImage) previewImage.removeAttribute('data-updated');
    });

    document.body.addEventListener('click', async function (e) {
        const btnCrop = e.target.closest('#btnCrop');
        if (!btnCrop || !cropper) return;
